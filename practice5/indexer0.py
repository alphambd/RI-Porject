...
class WeightedInvertedIndex:
    ...
    def extract_xml_elements_with_etree(self, xml_file_path, target_tags=['bdy', 'sec', 'p']):
        """Extrait les éléments XML avec ElementTree (structuré)"""
        try:
            # Parser le XML avec ElementTree
            tree = ET.parse(xml_file_path)
            root = tree.getroot()
            
            # Obtenir l'ID du document
            doc_id = self._extract_doc_id_from_elementtree(root, xml_file_path)
            
            elements = []
            element_counter = {}
            
            def traverse_element(elem, parent_path="", depth=0):
                """Parcours récursif de l'arbre XML"""
                current_tag = elem.tag.lower() if hasattr(elem, 'tag') else str(elem)
                
                # Compter les occurrences de ce tag à ce niveau
                if parent_path not in element_counter:
                    element_counter[parent_path] = Counter()
                element_counter[parent_path][current_tag] += 1
                tag_index = element_counter[parent_path][current_tag]
                
                # Construire le chemin XPath
                current_path = f"{parent_path}/{current_tag}[{tag_index}]"
                ...
                # Si c'est une balise cible ET a suffisamment de contenu
                if current_tag in target_tags and text_content and len(text_content) > 10:
                    # Générer ID unique basé sur le chemin
                    path_hash = hashlib.md5(current_path.encode()).hexdigest()[:8]
                    element_id = f"{doc_id}_{path_hash}"
                ...
                for child in elem:
                    traverse_element(child, current_path, depth + 1)
            
            # Démarrer le parcours
            traverse_element(root)
            
            # Éliminer les doublons et problèmes hiérarchiques
            unique_elements = self._filter_hierarchical_duplicates(elements)
            
            return unique_elements
            
        except ET.ParseError as e:
            print(f"Erreur de parsing XML {xml_file_path}: {e}")
            # Fallback à l'ancienne méthode regex si nécessaire
            return self.extract_xml_elements(xml_file_path, target_tags)
        except Exception as e:
            print(f"Erreur inattendue {xml_file_path}: {e}")
            return []
    ...
    def _extract_doc_id_from_elementtree(self, root_element, file_path):
        """Extrait l'ID du document depuis l'arbre ElementTree"""
        ...
        return os.path.basename(file_path).replace('.xml', '')
    ...
    def _extract_text_from_element(self, element):
        ...
        # Ajouter le texte de tous les enfants
    ...
    def _filter_hierarchical_duplicates(self, elements):
        """Filtre les éléments pour éviter les chevauchements hiérarchiques"""
        # Trier par profondeur (du plus profond au plus superficiel)
        elements_sorted = sorted(elements, key=lambda x: x['depth'], reverse=True)
        
        filtered_elements = []
        seen_texts = set()
        
        for elem in elements_sorted:
            # Vérifier si cet élément est contenu dans un élément déjà sélectionné
            is_contained = False
            for selected in filtered_elements:
                # Si le chemin de l'élément courant commence par le chemin d'un élément déjà sélectionné
                # ET qu'ils sont dans le même document
                if (elem['doc_id'] == selected['doc_id'] and 
                    elem['xml_path'].startswith(selected['xml_path'] + '/')):
                    is_contained = True
                    break
            
            # Éviter aussi les textes identiques
            text_hash = hashlib.md5(elem['text'].encode()).hexdigest()
            
            if not is_contained and text_hash not in seen_texts:
    ...
    def build_index_from_xml_elements_v2(self, xml_dir, target_tags=['bdy', 'sec', 'p'], max_files=None):
        """Indexe les éléments XML avec ElementTree (version améliorée)"""
        start_time = time.time()
        
        print(f"Indexation des éléments {target_tags} avec ElementTree...")
        ...
        total_elements = 0
        stats_by_tag = Counter()
        
        for i, xml_file in enumerate(xml_files):
            if i % 100 == 0:
                print(f"  Traitement du fichier {i+1}/{len(xml_files)}")
            
            # Utiliser ElementTree au lieu de regex
            elements = self.extract_xml_elements_with_etree(xml_file, target_tags)
            
            for elem_data in elements:
                elem_id = elem_data['elem_id']
                elem_text = elem_data['text']
                
                # Ne pas indexer les éléments trop courts
                if len(elem_text.split()) < 3:  # Moins de 3 mots
                    continue
                ...
                if self._index_document_content(elem_id, elem_text):
                    self.store_metadata(elem_id, {
                        ...
                        'parent_path': elem_data.get('parent_path', ''),
                        'source_file': elem_data['file_path']
                    })
                    total_elements += 1
                    stats_by_tag[elem_data['tag']] += 1
        ...
        print(f"\nIndexation terminée: {total_elements} éléments indexés")
        print(f"  Répartition: {dict(stats_by_tag)}")
        print(f"  Temps d'indexation: {indexing_time:.2f} secondes")
        
        return indexing_time

    def extract_xml_elements_with_namespaces(self, xml_file_path, target_tags):
        """Gestion des namespaces XML (très important pour XML scientifique)"""
        try:
            # Enregistrer les namespaces
            namespaces = {}
            
            # Parcourir pour trouver les namespaces
            for event, elem in ET.iterparse(xml_file_path, events=('start-ns',)):
                namespace, uri = elem
                namespaces[namespace] = uri
            
            # Parser avec gestion des namespaces
            tree = ET.parse(xml_file_path)
            root = tree.getroot()
            
            # Créer un mapping pour les recherches
            nsmap = {v: k for k, v in namespaces.items()}
            
            # Fonction pour chercher avec namespaces
            def find_with_ns(tag_name):
                ...
                # Chercher avec chaque namespace
            ...
            # Utiliser cette fonction pour extraire les éléments cibles
            elements = []
            for tag in target_tags:
                for elem in find_with_ns(tag):
                    # Traitement similaire à la version sans namespaces
                    pass
            
            return elements
            
        except Exception as e:
            print(f"Erreur avec namespaces {xml_file_path}: {e}")
            # Retourner à la version sans namespaces
            return self.extract_xml_elements_with_etree(xml_file_path, target_tags)
