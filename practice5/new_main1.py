...
def get_elements_for_article(article_id, query_terms, element_ranker, score_threshold=0.0):
    ...
    Args:
        ...
        score_threshold: Seuil minimal de score (optimisation)
    ...
    for element_id in element_ranker.index.doc_ids:
        ...
        if str(metadata.get('parent_doc_id', '')) == str(article_id):
            ...
            for term in query_terms:
                weight = element_ranker.get_term_weight_cached(term, element_id, "ltn")
                if weight is not None:
                    score += weight
            
            # Filtrer par seuil de score (optimisation)
...
def main():
    """Fonction principale avec les approches optimisées"""
    ...
    # Créer le répertoire runs
    os.makedirs(runs_dir, exist_ok=True)
    
    # Configuration de base pour les index
    base_config = {
        'tokenization': 'basic',
        'stemmer': 'nostem',
        'stop_words': 'nostop'
    ...
    print("\n" + "="*80)
    print("SYSTEME DE RECHERCHE INEX - VERSIONS OPTIMISEES")
    print("="*80)
    
    # Création des index (avec cache)
    print("\nCREATION DES INDEX (avec cache)...")
    ...
    run_file = generate_fetch_browse_pooling_optimized(
        ...
        progress_interval=250  # Moins de messages de progression
    )
