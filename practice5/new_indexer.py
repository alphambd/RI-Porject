...
class INEXDocument:
    ...
    def _normalize_xpath(self, path: str) -> str:
        """
        Ajoute des indices [1] aux balises qui n'en ont pas pour 
        être conforme au format strict attendu par les évaluateurs.
        """
        parts = path.split('/')
        normalized_parts = []
        ...
        for part in parts:
            if not part:
                continue
            # Si la balise n'a pas d'indice (pas de crochet), on ajoute [1]
            if '[' not in part:
                normalized_parts.append(f"{part}[1]")
            else:
    ...
    def get_inex_elements(self, target_tags: Set[str]) -> List[Dict]:
        ...
        for elem in self.tree.iter():
            ...
            # On ne traite que les balises cibles (p, sec, bdy, article)
            if tag in target_tags or tag == 'article':
                text = self._extract_element_text(elem)
                
                # Filtre de taille pour éviter les "small_err_nodes"
                # On augmente le seuil pour être plus sélectif
                min_length = 50 if tag != 'p' else 20
                
                if text and len(text.strip()) > min_length:
                    # 1. RÉCUPÉRATION DU CHEMIN NATIF via lxml
                    raw_path = self.tree.getpath(elem)
                    
                    # 2. NORMALISATION (Ajout des [1] manquants)
                    clean_path = self._normalize_xpath(raw_path)
                    
                    # 3. FILTRAGE DE SÉCURITÉ
                    # On s'assure que le chemin commence bien par /article
                    if not clean_path.startswith('/article'):
                        continue

                    element_id = f"{self.doc_id}_{hashlib.md5(clean_path.encode()).hexdigest()[:8]}"
    ...
    def get_inex_elements(self, target_tags: Set[str], approach: str = 'A') -> List[Dict]:
        elements = []
        if self.tree is None:
            return elements
        ...
        for elem in self.tree.iter():
            ...
            if tag in target_tags or tag == 'article':
                ...
                # Filtre de taille minimal pour éviter les "Small"
                if text and len(text.strip()) > 50:
                    ...
                    if approach == 'A':
                        # Approche A : On filtre. Si pas standard, on passe au suivant.
                        if not self.is_path_standard(clean_path):
                            continue
                            
                    elif approach == 'B':
                        # Approche B : On simplifie le chemin.
                        #clean_path = self.simplify_xpath(clean_path)
                        clean_path = self.simplify_xpath(clean_path)
                    
                    # Sécurité finale : toujours vérifier que ça commence par /article
                    if not clean_path.startswith('/article'):
                        continue
    ...
    def is_path_standard(self, normalized_path: str) -> bool:
        """
        APPROCHE A : Ne garde que les chemins composés de balises documentaires classiques.
        """
        # Liste des balises autorisées par les évaluateurs standards
        allowed_tags = {'article', 'bdy', 'sec', 'ss1', 'ss2', 'p', 'title', 'it', 'list', 'item'}
        
        # On découpe le chemin et on enlève les indices [n] pour vérifier le nom du tag
        # Exemple: "/article[1]/bdy[1]/sec[2]" -> ['article', 'bdy', 'sec']
        parts = [part.split('[')[0] for part in normalized_path.split('/') if part]
        
        # Vérifie si chaque segment du chemin est dans la liste autorisée
        return all(tag in allowed_tags for tag in parts)

    def simplify_xpath(self, normalized_path: str) -> str:
        """
        APPROCHE B - OPTION CONTENEUR AMÉLIORÉE :
        Gère la simplification tout en respectant la cible (article, sec ou p).
        """
        # 1. Normalisation du début (Saut des balises sémantiques)
        if '/bdy[' in normalized_path:
            match = re.search(r'(/bdy\[\d+\].*)', normalized_path)
            if match:
                normalized_path = f"/article[1]{match.group(1)}"
        
        # 2. Découpage pour analyse
        parts = [p for p in normalized_path.split('/') if p]
        simplified_parts = []
        
        # On identifie le tag cible (le dernier élément du chemin original)
        # Exemple: si path finit par p[3], la cible est p
        target_part = parts[-1]
        
        found_main_sec = False
        
        for part in parts:
            # Garder la structure racine
            if 'article[' in part or 'bdy[' in part:
                simplified_parts.append(part)
            
            # Garder la PREMIÈRE section rencontrée (le conteneur principal)
            elif 'sec[' in part and not found_main_sec:
                simplified_parts.append(part)
                found_main_sec = True
                
            # Si la cible est elle-même une sous-section (ss1, ss2), 
            # on la garde mais on peut la renommer en 'sec' pour la conformité
            elif 'ss' in part and part == target_part:
                clean_ss = re.sub(r'ss\d+', 'sec', part)
                simplified_parts.append(clean_ss)
                
            # Garder le paragraphe si c'est la cible
            elif 'p[' in part:
                simplified_parts.append(part)

        # 3. Nettoyage des doublons de segments 
        # (Évite /article[1]/article[1] si le chemin était court)
        final_parts = []
...
class WeightedInvertedIndex:
    ...
    def build_index_from_elements(self, xml_dir: str, 
        ...
        for i, xml_file in enumerate(xml_files):
            ...
            elements = doc.get_inex_elements(list(tag_set), 'B')
