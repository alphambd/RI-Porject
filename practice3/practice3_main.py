...
def compute_statistics(exercise_num, stop_words_set=None, use_stop_words=False, use_stemmer=False):
    ...
    if stop_words_set is None:
        stop_words_set = set()  # pas de stop-words

    index = WeightedInvertedIndex(stop_words_set=stop_words_set)
...
def main():
    """Fonction principale"""
    # Charger le fichier stop-words UNE SEULE FOIS
    stop_words_file = "data/stop-words-english4.txt"
    try:
        with open(stop_words_file, 'r', encoding='utf-8') as f:
            stop_words_set = set(line.strip().lower() for line in f if line.strip())
        print(f"- {len(stop_words_set)} stop words chargés")
    except FileNotFoundError:
        stop_words_set = set()
        print(f"Fichier stop-words non trouvé: {stop_words_file}")

    # Exercice 1: sans traitement de tokens
    index1 = compute_statistics(1, stop_words_set=None, use_stop_words=False, use_stemmer=False)

    # Exercice 2: avec stop-words et stemming
    index2 = compute_statistics(2, stop_words_set=stop_words_set, use_stop_words=True, use_stemmer=True)
