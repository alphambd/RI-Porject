...
class InvertedIndex:
    ...
    def add_document(self, doc_id, text, filename=None):
        ...
        word = re.sub(r"[^a-zA-Z\s]", "", text).split()
        ...
        self.fils_doc_words+= word
        self.fils_doc_terms+= term_freq
    ...
    def build_from_file(self, filename, print_index=False):
        ...
        for doc_id, doc_text in matches:
            doc_id = doc_id.strip()
            doc_text = doc_text.strip()
            self.fils_doc_ids.append(doc_id)
    ...
    def get_data(self):
        ...
        return (self.fils_doc_words, self.fils_doc_terms, self.fils_doc_ids)

    def get_smart_itn(self) :
        """Retourne le smart itn"""
        res= [[] for term in set(self.fils_doc_terms)]
        N= len(self.fils_doc_ids)
        i = 0
        count_terms = len(set(self.fils_doc_terms))
        for term in set(self.fils_doc_terms):
            df_t = len(self.dictionary[term])
            for doc_id in sorted(self.fils_doc_ids):
                if (doc_id in self.dictionary[term]):
                    res[i].append(self.smart_ltn_weight(self.dictionary[term][doc_id], df_t,N))
                res[i].append(0)
            i +=1
        return res


    def smart_ltn_weight(self,tf_td, df_t, N):
        """
        Calcule le poids SMART ltn d’un terme t dans un document d.
        ...
        Retour :
        --------
        float : poids w_{t,d} selon le schéma SMART ltn
        """

        if tf_td <= 0 or df_t == 0:
            return 0.0

        # 1. Pondération logarithmique du tf
        tf_weight = 1 + math.log10(tf_td)

        # 2. Pondération IDF (inverse document frequency)
        idf_weight = math.log10(N / df_t)

        # 3. Pas de normalisation pour "n"
